...
### 11.8  variance exponent moment cascade都有啥不同

这四类不是同一层东西，先分一下类再比，不然容易混：

- **Cascade（CSM）**：属于“怎么把阴影图覆盖大场景”的**空间划分**方案，跟用什么滤波无关。
- **Variance / Exponential / Moment**：属于“阴影图里存什么、怎么比较、怎么软化”的**滤波/重建**方案，可以单独用，也可以叠在 CSM 上。

下面分开说。

#### 一、Cascaded Shadow Map（CSM）

目的不是软化阴影，而是解决方向光/太阳在大视距下的精度问题。

做法：
- 按相机视锥从近到远切成几段（cascade 0/1/2/3…）
- 每段单独算一个光源正交投影，渲一张深度图
- 近段分辨率高、远段分辨率低
- 着色时根据像素深度选对应 cascade，做常规深度比较（或再接 PCF/VSM 等）

特点：
- 硬边时通常配 PCF；想软边也可每段存 VSM/ESM/MSM
- 主要解决“远距离锯齿、近处不够细”
- 开销随 cascade 数线性增加，通常 2–4 级
- UE 老方向光、Unity 方向光默认思路都是它；UE5 VSM 是另一种替代，不等于 CSM

> 结论：CSM 是“分块渲染”，VSM/ESM/MSM 是“怎么存深度分布、怎么滤波”。

#### 二、Variance Shadow Map（VSM）

传统 SM 只存最近深度 z，不能先模糊再比较。VSM 改存两个值：

- m₁ = 平均深度 E[z]
- m₂ = 平均深度平方 E[z²]

存在一张 RG 浮点图里，可以先做高斯模糊，再按 Chebyshev 不等式估可见性：
...
缺点：
- **light bleeding** 明显：漏光，多遮挡叠加处会发灰发亮
- 深度平方动态范围大，需要浮点/高精度，否则精度差
- 对 bias 敏感，近处薄物体容易出问题
- 只存两矩，对复杂遮挡近似较粗

工程中常加：深度归一化到 [0,1]、clamp、减小 far/near 跨度，或用 EVSM 改进。

#### 三、Exponential Shadow Map（ESM）

思路更激进：不存深度本身，存深度的指数矩。

通常存：
- M = E[e^{c·z}] 或 E[e^{−c·z}]（c 为缩放常数）

同样可以先模糊，再比较接收点指数值和图中指数值。

优点：
- 只存 1 个通道，比 VSM 更省（理论上）
- 非常容易线性滤波、大半径模糊
- 软边质量在单矩方法里算不错，实现比 MSM 简单

缺点：
- c 很敏感：太小漏光，太大数值爆炸
- e^{c·z} 动态范围极大，必须用 HDR/浮点，否则精度崩
- 多遮挡、复杂几何仍有 light bleeding 和过柔
- 实际调参比 VSM 更烦，很多项目不用纯 ESM

变体：
- EVSM（Exponential Variance）：存指数后的均值和方差，兼顾 VSM 稳定与 ESM 软化，light bleeding 比纯 VSM 小。
- 还有一些 log-evsm 思路。

#### 四、Moment Shadow Map（MSM）

VSM 存 2 个矩，ESM 存 1 个指数矩；MSM 存更多阶矩来重建被遮概率。

常见：
- 4-moment：1, z, z², z³ 或优化后的一组矩
- 6-moment 更高阶
- 通过矩重建近似深度 CDF，解一个最小二乘/边界问题得到可见性

优点：
- light bleeding 明显少于 VSM/ESM
- 可预滤波，支持大半径软边
- 对薄物体、多层遮挡、非平面接收面更稳
- 质量接近“花钱买精度”的软阴影方案

缺点：
- 带宽和显存高：4 通道甚至 6 通道浮点
- shader 重建 ALU 更重，实现复杂
- 需要做好矩的正则化、防止数值病态
- 不是所有引擎默认开，适合高端 PC/主机

#### 五、横向对比

| 方案 | 存什么 | 可预滤波 | 软边质量 | light bleeding | 成本/带宽 | 典型用途 |
|---|---|---|---|---|---|---|
| 传统SM+PCF | 最近深度 | 否（采样时滤波） | 中，靠多采样 | 低 | 低 | 通用默认、移动端 |
| CSM | 分块深度图 | 取决于下层滤波 | 取决于PCF/VSM等 | 取决于下层 | 中 | 大世界方向光 |
| VSM | 深度、深度平方 | 可 | 中～好 | 明显 | 中，2通道浮点 | 软阴影、大范围半影 |
| ESM | 单指数矩 | 可 | 中～好 | 中～明显 | 低通道但要高动态浮点 | 少用在生产，研究/特定管线多 |
| EVSM | 指数方差/双指数矩 | 可 | 好 | 少于VSM | 中高 | 想替代VSM时常用 |
| MSM | 4/6阶矩 | 可 | 很好 | 很少 | 高，4–6通道浮点 | 高端软阴影、面积光近似 |

#### 六、实际怎么选

- 移动/全平台/少折腾：方向光 CSM + PCF；点/聚光单图 + PCF。
- PC/主机要软边且不想写太复杂：CSM + VSM，或 CSM + EVSM；注意调 depth range 和 bias。
- 大世界、近处要求高、UE5：可以考虑 Virtual Shadow Maps，不必自己堆 CSM+VSM。
- 高端电影化、面积光半影、重要主光：MSM，或局部上光线追踪阴影。
- 纯 ESM 不建议新手直接上，调 c 和浮点格式会很痛苦；真要指数思路用 EVSM 更稳。

如果你愿意，我可以按“Unity URP/HDRP”或“UE5 自研延迟渲染”给一套 cascade 级数 + VSM/MSM 通道格式 + bias 参数模板。
