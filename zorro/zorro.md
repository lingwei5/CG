...
16. 红外模型需要尽量朝外
17. 地形的红外RGBA四通道纹理分解成四个mask，分别导入地形;草、树木的四通道红外纹理直接使用即可;mesh需要先根据.dat生成.obj,再人工确认模型朝向，修改完成后再次根据.obj生成带uv的.ob及四通道红外纹理 后期看看能不能.dat输出带法线的保证朝向，这样运行一次成像即可完成整个红外纹理生成
...
12. 树木用2,避免榕树枝条的黑边
13. 地形的红外RGBA四通道纹理分解成四个mask，分别导入地形;草、树木的四通道红外纹理直接使用即可;mesh需要先根据.dat生成.obj,再人工确认模型朝向，修改完成后再次根据.obj生成带uv的.ob及四通道红外纹理 后期看看能不能.dat输出带法线的保证朝向，这样运行一次成像即可完成整个红外纹理生成
